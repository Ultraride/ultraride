    ...
    if (withCoords.length === 0) {
      ...
      withCoords.forEach((r) => {
        const marker = L.circleMarker([r.lat, r.lon], {
          radius: 7,
          weight: 2,
          color: r.open ? "#E3A23C" : "#C1543F",
          fillColor: r.open ? "#E3A23C" : "#C1543F",
          fillOpacity: r.open ? 0.85 : 0.15,
        });
        marker.bindTooltip(
          `<div style="font-family:'Oswald',sans-serif;font-weight:600;font-size:13px;">${r.name}</div><div style="font-family:'IBM Plex Mono',monospace;font-size:11px;color:#B9C3B4;margin-top:2px;">${r.km ?? "—"} km · ${r.dplus ? r.dplus.toLocaleString("fr-FR") : "—"} D+ · ${r.month ?? ""}</div>`,
